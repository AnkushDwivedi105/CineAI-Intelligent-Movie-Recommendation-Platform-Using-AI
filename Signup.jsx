...
const Signup = () => {
  ...
  return (
    <div className="min-h-screen bg-bg-dark flex flex-col items-center justify-center relative overflow-hidden px-4">
        ...
        <div className="glass-card p-10 rounded-3xl border-white/5 shadow-2xl">
          ...
          {error && (
            <div className="bg-brand/10 border border-brand/20 p-4 mb-6 rounded-xl text-brand text-xs font-bold text-center">
              ...
              {error.includes('taken') && (
                <div className="mt-2">
                  <Link to="/login" className="underline text-white">Login instead?</Link>
                </div>
              )}
            </div>
          )}
          ...
          <div className="mt-8 text-center">
            <p className="text-white/40 text-sm font-medium">
              Already have an account? <Link to="/login" className="text-brand font-black hover:underline transition">Sign In</Link>
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Footer Branding */}
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 opacity-20 flex gap-4 text-[10px] font-black uppercase tracking-widest text-center">
